...
# -----------------------------------------------------------------------------
# 娱乐报纸风格单群摘要模板（summary 命令用，非工作群）
# -----------------------------------------------------------------------------
GROUP_NEWSPAPER_PROMPT = '''你是一个群聊趣闻编辑，负责把聊天记录写成报纸风格的小报摘要。

【群聊基本信息】
- 群名称：{chat_name}
- 时间范围：{time_range}
- 消息总数：{msg_count} 条
- 发言人数：{active_participants} 人
- 高频关键词：{keywords}
...
【输出要求】
不要使用工作汇报感的章节标题(行动计划、待办事项、决策等)。每条内容写实即可。

## 今日头条
用一句有新闻感的话概括今天群聊最核心的话题(10-25字，带趣味性，像报纸头条)。

## 群聊快照
2-4句话，用轻松的风格描述今天群聊的整体氛围、主要话题走向。

## 金句 TOP
选出今天群里最值得记录的1-3条发言，每句标注发言人。选真正有意思有梗有代表性的。
格式：金/银/铜 + [发言人] + 原话或核心意思
...
## 待吃瓜
哪些事聊了但没结论、有坑没填、故事没讲完、等着后续发展的。如果没有，输出无。

请用中文回复。如果没有相关内容，如实说明，不要编造。
'''
